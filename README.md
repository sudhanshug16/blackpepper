...
`opencode` on the host where it will run. When an exact Zellij or Worktrunk is
not available, Blackpepper downloads the pinned archive, checks its embedded
SHA-256, and installs a private versioned sidecar without editing `PATH` or
shell files.
